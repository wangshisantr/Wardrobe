...
<body>
	...
	<div id="wrapper">
		...
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="index.html">BOOT客户管理系统 v1.0</a>
		</div>
		<!-- /.navbar-header -->
		...
		<ul class="nav navbar-top-links navbar-right">
			<li class="dropdown"><a class="dropdown-toggle"
				...
				<ul class="dropdown-menu dropdown-messages">
					<li><a href="#">
							<div>
								<strong>管理员</strong> <span class="pull-right text-muted">
									<em>昨天</em>
								</span>
							</div>
							<div>为了密码安全，第一次登录请修改密码</div>
					</a></li>
					<li class="divider"></li>
			...
			<li class="dropdown"><a class="dropdown-toggle"
				...
				<ul class="dropdown-menu dropdown-user">
					...
					<li class="divider"></li>
					<li><a href="${pageContext.request.contextPath }/user/logout"><i class="fa fa-sign-out fa-fw"></i>
							退出登录</a></li>
				</ul> <!-- /.dropdown-user --></li>
			<!-- /.dropdown -->
		</ul>
		...
		<div class="navbar-default sidebar" role="navigation">
			<div class="sidebar-nav navbar-collapse">
				<ul class="nav" id="side-menu">
					...
					<li><a href="${pageContext.request.contextPath }/customer/count" class="active"><i
							class="fa fa-edit fa-fw"></i> 客户统计</a></li>
				</ul>
			</div>
			<!-- /.sidebar-collapse -->
		</div>
				...
				<div class="panel-body">
					<form class="form-inline" action="${pageContext.request.contextPath }/customer/list.action" method="get">
						<div class="form-group">
							...
							<input type="text" class="form-control" id="customerName" value="${custName }" name="custName" placeholder="支持模糊查询">
						</div>
						<div class="form-group">
						...
						<a class="btn btn-primary" data-toggle="modal" data-target="#customerEditDialog" 
						onclick="addCustomer()">新增</a>
						<a href="#" class="btn btn-primary " onclick="checkAll()">全选</a>
						<a href="#" class="btn btn-primary " onclick="checkReverse()">反选</a>
						<a href="#" class="btn btn-danger " onclick="deleteAllCustomer()">批量删除</a>
					</form>
				</div>
			</div>
			<div class="row">
							...
							<table class="table table-bordered table-striped table-hover">
								...
								<tbody>
									<c:forEach items="${page.rows}" var="row">
										<tr>	
											<td><input type="checkbox" name="checkedId" value="${row.cust_id }"></td>
											<td>${row.cust_name}</td>
											...
											<td>
												<a href="#" class="btn btn-primary " data-toggle="modal" data-target="#customerEditDialog" onclick="editCustomer(${row.cust_id})">修改</a>
												<a href="#" class="btn btn-danger " onclick="deleteCustomer(${row.cust_id})">删除</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" id="submitModal" class="btn btn-primary" >确认</button>
					<!-- <button type="button" id="submitModal" class="btn btn-primary" onclick="updateCustomer()">确认</button> -->
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function deleteAllCustomer(){
			...
			$("input[type='checkbox']").each(function(){ 
				//判断是否被选中 if($("#id").is(":checked")==true）or if($("#id").prop("checked")==true）
				//动态选中一个checkbox 值  $("#id").attr("checked"，"true")
				//取消选中 $("#id").removeAttr("checked")
			...
			if(!flag){
				...
				if(confirm('确实要删这些该客户吗?')) {
					document.itemsForm.action = "${pageContext.request.contextPath }/customer/deleteAll";
		...
		function checkAll(){
			$("input[type='checkbox']").each(function(){ 
				//判断是否被选中 if($("#id").is(":checked")==true）or if($("#id").prop("checked")==true）
				//动态选中一个checkbox 值  $("#id").attr("checked"，"true")或者$("#id").prop("checked"，"true")
				//取消选中 $("#id").removeAttr("checked")
				// $("#id").attr("checked"，"true")只能用一次，$("#id").prop("checked"，"true")可多次使用
				if(!$(this).prop("checked")) {
